...
class AccountingSchema(BaseModel):
    ...
    entries: List[AccountingEntrySchema] = Field(
        default_factory=list, description="Accounting entries"
    )
...
class EventCreate(BaseModel):
    ...
    event_metadata: Optional[EventMetadataSchema] = Field(
        None, description="Event metadata"
    )

    accounting: Optional[AccountingSchema] = Field(
        None, description="Accounting entries"
    )
    tenant_id: Optional[str] = Field(None, description="Tenant identifier")
...
class EventSubscriptionCreate(BaseModel):
    ...
    event_types: Optional[List[EventType]] = Field(
        None, description="Event types to subscribe"
    )
    tenant_id: Optional[str] = None
