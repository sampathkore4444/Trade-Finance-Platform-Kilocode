...
def create_event(
    ...
    try:
        ...
        if event_data.metadata:
            metadata = event_data.metadata.model_dump()
...
@router.get("/{event_id}/accounting-status", response_model=EventAccountingStatusResponse)
...
def create_subscription(
    ...
    subscription = EventSubscription(
        ...
        event_types=[et.value for et in subscription_data.event_types] if subscription_data.event_types else None,
...
def health_check(
    ...
    return EventHealthResponse(
        status="healthy" if all(s == "ok" for s in [event_store_status, message_queue_status, event_generator_status]) else "degraded",
